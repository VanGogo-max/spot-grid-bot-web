...
def dashboard():
    ...
    # 🔴 НОВ КОД: Автоматично активиране на абонамент за админ
    if user.get('is_admin'):
        if not user['subscription_active']:
            activate_subscription(user_id, months=9999)  # 9999 месеца = завинаги
            user = get_user_by_id(user_id)  # Презареди потребителските данни
...
def admin_make_admin(user_id):
    """Направи потребител админ"""
    result = set_user_as_admin(user_id)
    
    if result['success']:
        flash(f'✅ Потребител {user_id} е сега админ!', 'success')
    else:
        flash('❌ Грешка при правене на админ!', 'error')
